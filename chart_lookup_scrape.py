...
def lookup_metadata(apple_ids: List[int], country: str) -> Dict[int, dict]:
    ...
    def scrape_html(aid):
        url = f"https://podcasts.apple.com/{country}/podcast/id{aid}"
        try:
            ...
            title = title["content"].strip() if title and title.get("content") else ""

            artwork = soup.find("meta", property="og:image")
            artwork = (
                artwork["content"].strip() if artwork and artwork.get("content") else ""
            )

            page_url = soup.find("meta", property="og:url")
            page_url = (
                page_url["content"].strip()
                if page_url and page_url.get("content")
                else url
            )

            if title:
                return {"title": title, "artwork": artwork, "podcast_url": page_url}
        except Exception as e:
            print(f"⚠️ HTML scrape failed for {aid}: {e}")
        return None

    # Step 1: Try normal lookup in batches
