...
def main():
    ...
    if last_table and last_table.startswith(f"apple_chart_{today_str}"):
        ...
        if completed_regions < len(REGIONS):
            ...
            print(
                f"🔄 Resuming into existing table: {current_table} (completed {completed_regions}/{len(REGIONS)} regions)"
            )
    if not reuse_existing:
        ...
        try:
            ...
            if previous_table:
                print(f"📊 Will compare with: {previous_table}")
        except mysql.connector.Error as e:
    ...
    # ----- Resume from the last completed region -----
    ...
    if done_regions:
        ...
        if last_done:
            ...
            print(f"🔁 Resuming from region {last_done.upper()} (wiped its old rows)")

    total_regions = len(REGIONS)
    for region_idx, region in enumerate(REGIONS[start_index:], start=start_index + 1):
        print(
        ...
        # Commit per region (idempotent): first clear, then insert
        cursor.execute(f"DELETE FROM {current_table} WHERE countryCode = %s", (region,))
    ...
    # ----- Run rank comparison ONLY when table is complete -----
    cursor.execute(f"SELECT COUNT(DISTINCT countryCode) FROM {current_table}")
    completed_regions = cursor.fetchone()[0]
    print("\n📊 Rank comparison gate check...")
    if completed_regions == len(REGIONS) and previous_table:
        ...
        if completed_regions != len(REGIONS):
            print(
                f"⏸️ Skipping rank comparison – only {completed_regions}/{len(REGIONS)} regions completed"
            )
        elif not previous_table:
            print("⏸️ Skipping rank comparison – no previous table available")
    ...
    print(f"  • Created/used table: {current_table}")
    if previous_table:
        print(f"  • Compared with: {previous_table}")
    if failed_requests:
