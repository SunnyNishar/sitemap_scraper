...
def fetch_chart_ids(region: str, genre_id: int, limit: int) -> List[int]:
    ...
    data = res.json()
    # Charts returns "resultIds" in rank order
...
def get_amp_token(country="us"):
    """
    Fetch AMP API token from the Apple Podcasts browse page.
    """
    browse_url = f"https://podcasts.apple.com/{country}/browse"
    r = requests.get(browse_url, headers={"User-Agent": "Mozilla/5.0"})
    r.raise_for_status()

    # Token is usually in JSON: "mediaApiToken":"..."
    match = re.search(r'"mediaApiToken"\s*:\s*"([^"]+)"', r.text)
    if match:
        return match.group(1)

    # Fallback: token might be escaped
    match = re.search(r'\\"mediaApiToken\\":\\"([^\\]+)\\"', r.text)
    if match:
        return match.group(1)

    raise RuntimeError("No AMP API token found in page HTML")


def fetch_jsonld_metadata(apple_id, country):
    """
    Fetch podcast metadata from the podcast's JSON-LD block in HTML.
    """
    url = f"https://podcasts.apple.com/{country}/podcast/id{apple_id}"
    ...
    try:
        ...
        if match:
            jsonld_str = match.group(1).strip()
            data = json.loads(jsonld_str)

            return {
                "title": data.get("name", "").strip(),
                "artwork": data.get("image", ""),
                "podcast_url": data.get("url", url),
            }
    except Exception as e:
        print(f"⚠️ JSON-LD fetch failed for {apple_id}: {e}")
...
def lookup_metadata(apple_ids: List[int], country: str) -> Dict[int, dict]:
    ...
    # --- Step 1: Lookup API in batches ---
    def do_lookup(ids_chunk):
        ids_str = ",".join(str(x) for x in ids_chunk)
        ...
        try:
            ...
            print(f"⚠️ Lookup failed for {ids_str[:80]}...: {e}")
            return []

    for chunk in batched(apple_ids, 100):
        found_ids = set()
        for item in do_lookup(chunk):
            ...
            out[aid] = {
                ...
                "podcast_url": item.get("trackViewUrl")
    ...
    # --- Step 2: JSON-LD scrape for missing IDs ---
    if missing_ids:
        for aid in missing_ids:
            meta = fetch_jsonld_metadata(aid, country)
            if meta:
                out[aid] = meta
            else:
                out[aid] = {
                    "title": f"Unknown Podcast (ID: {aid})",
                    "artwork": "",
                    "podcast_url": f"https://podcasts.apple.com/{country}/podcast/id{aid}",
                }
