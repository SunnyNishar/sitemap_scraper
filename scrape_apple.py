        ...
        try:
            ...
            if isinstance(feed_data, dict):
                ...
                if isinstance(feed, dict):
                    ...
                    print(
                        f"⚠️ Feed is not a dict for {region.upper()} | {genre_id}, but continuing..."
                    )
            else:
                print(
                    f"⚠️ Unexpected response format for {region.upper()} | {genre_id}, but continuing..."
                )
                # Try to find entries in the response anyway
...
# Save Apple IDs
output_path = "./documents/apple_ids_from_all_genres_regions2.txt"
